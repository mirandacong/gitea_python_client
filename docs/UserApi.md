...
try:
    # Create an access token
    api_instance.user_create_token(username)
except ApiException as e:
...
> list[PublicKey] user_current_list_keys()
...
api_instance = swagger_client.UserApi(swagger_client.ApiClient(configuration))
username = 'username_example' # str | username of user
token = 56 # int | token to be deleted
...
# **user_list_keys**
> list[PublicKey] user_list_keys(username)
...
# **user_search**
> InlineResponse200 user_search(q=q, limit=limit)
...
try:
    # Search for users
    api_response = api_instance.user_search(q=q, limit=limit)
    pprint(api_response)
except ApiException as e:
